...
disp('✅ All jobs submitted and job table saved.');
...
for s = 1:numel(tiSizes)
	...
	for iter = 1:n_iter
		...
		for i = 1:size(iter_jobs,1)
			...
			try
				...
				fprintf('✅ Job %s done (%.2fs)\n', jobid, t);
			catch ME
				warning('⚠️ Failed to download %s: %s', jobid, ME.message);
			end
		end
	end
	...
	fprintf('✅ Saved results for TI = %dx%d\n', currSize, currSize);
end
...
%% ============================================================
%  PHASE 3 — AGGREGATION & RELATIVE GAIN COMPUTATION
%  ============================================================
...
%% ============================================================
%  PHASE 4 — VISUALIZATION WITH 5–95 PERCENTILES (FIXED)
%  ============================================================
...
gainHigh = gainHigh(:);
gainRatio = gainRatio(:);
meanTime = meanTime(:,:); % keep as 2D

% --- Plot ---
f = figure('Name','Gain vs TI size','Position',[100 100 950 750],'Visible','on');
set(f, 'Color', 'w');

% Left axis → Relative gain (mean + 5–95 percentile)
yyaxis left
hold on

% Shaded 5–95 percentile range
x_fill = [TI_u; flipud(TI_u)];
y_fill = [gainLow; flipud(gainHigh)];
fill(x_fill, y_fill, [0.8 0.85 1], 'EdgeColor', 'none', 'FaceAlpha', 0.4);

% Mean line
plot(TI_u, gainRatio, '-o', ...
	'LineWidth', 1.8, 'Color', [0 0.2 0.8], ...
	'MarkerFaceColor', [0.2 0.4 1], 'MarkerSize', 5);
...
% Right axis → Mean runtime for reference
yyaxis right
...
legend({'5–95% range','Mean gain','Mean runtime (no-wPO)'}, ...
	'Location', 'northwest', 'Box', 'off');

drawnow;
exportgraphics(f, 'gain_scaling_vs_TI.png', ...
	'Resolution', 300, ...
	'BackgroundColor', 'none');
disp('✅ Visualization saved: gain_scaling_vs_TI.png');
